...
func _ready():
	...
	connect("score_updated", Callable(score_label, "_on_score_updated"))
...
func check_dice_values():
	...
	if rolled_values == sorted_target_values:
		...
		# Manually calculate the sum of rolled values
		var sum_of_rolled_values = 0
		for value in rolled_values:
			sum_of_rolled_values += value
		# Now, emit the signal with the sum of rolled values
		emit_signal("score_updated", sum_of_rolled_values)
